    ...
    <el-scrollbar class="scrollbar" style="height: 330px">
      ...
      >
        ...
        <el-table-column label="商品名称">
          <template #default="scope">
            {{ scope.row.penguin ? scope.row.penguin.penguinName : '-' }}
          </template>
        </el-table-column>

        <el-table-column label="当前数量">
          <template #default="scope">
            {{ scope.row.penguin ? (scope.row.currentCapacity !== null && scope.row.currentCapacity !== undefined ? scope.row.currentCapacity : '-') : '-' }}
          </template>
        </el-table-column>

        <el-table-column label="还可添加">
          <template #default="scope">
            {{ scope.row.penguin ? getAvailableCapacity(scope.row) : '-' }}
          </template>
        </el-table-column>
...
import { cancelTaskType } from '@/api/mind/taskType';
// 接口
// 获取货道接口
import { getGoodsList } from '@/api/mind/channel';
// 从父组件获取数据
...
const ensureDialog = () => {
  ...
  channelList.value.forEach((ele) => {
    if (ele.penguin && ele.expectCapacity > 0) {
      detailList.value.push({
        channelCode: ele.channelCode,
        expectCapacity: ele.expectCapacity,
        penguinId: ele.penguinId,
        penguinName: ele.penguin ? ele.penguin.penguinName : '',
        penguinImage: ele.penguin ? ele.penguin.penguinImage : '',
      });
    }
  });
