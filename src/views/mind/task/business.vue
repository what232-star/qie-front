    ...
    >
      ...
      >
        <template #default="scope">
          <span>{{ taskTypeList.find(item => item.typeId == scope.row.productTypeId)?.typeName || '-' }}</span>
        </template>
      </el-table-column>

      <el-table-column label="工单方式" align="center" prop="createType">
        <template #default="scope">
          <span>{{ task_create_type.find(item => item.value == scope.row.createType)?.label || '-' }}</span>
        </template>
      </el-table-column>
...
import { listTaskType } from '@/api/mind/taskType';
  ...
  getTaskDetails(taskId.value).then((res) => {
    ...
    detailData.value.map((taskDetail) => {
      obj = {
        ...
        penguinId: taskDetail.penguinId,
        penguinName: taskDetail.penguinName,
        penguinImage: taskDetail.penguinImage,
      };
      dataArr.push(obj);
    });
